...
const LadyPartnerForm: React.FC<LadyPartnerFormProps> = ({
  ...
  const handlePhoneChange = (value: string) => {
    updateLadyPartnerField(index, "phone", value);
  };

  // Specific handler for email input change
  const handleEmailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    updateLadyPartnerField(index, 'email', e.target.value);
    clearEmailConflictFlag('ladyPartner', index);
  };

  // Specific handler for email input blur
  const handleEmailBlur = (e: React.FocusEvent<HTMLInputElement>) => {
      setEmailInteracted(true); // Keep interaction tracking
      checkEmailOnBlur('ladyPartner', index, e.target.value);
  };

  const showContactFields = ladyPartner.contactPreference === "Directly";
  const showConfirmation =
    ladyPartner.contactPreference !== "Directly" &&
    ladyPartner.contactPreference !== "";
