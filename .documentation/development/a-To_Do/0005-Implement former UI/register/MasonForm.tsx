...
const MasonForm: React.FC<MasonFormProps> = ({
  ...
  // New Effect: Initialize/Sync Grand Lodge selection from context
  useEffect(() => {
    const contextGrandLodgeName = mason.grandLodge; // Get GL name from form context
    
    if (contextGrandLodgeName && grandLodges.length > 0) {
      // If we have a name from context and the GL list is loaded
      const found = grandLodges.find((gl: GrandLodgeRow) => 
          gl.name.toLowerCase() === contextGrandLodgeName.toLowerCase() ||
          (gl.abbreviation && gl.abbreviation.toLowerCase() === contextGrandLodgeName.toLowerCase())
      );
      
      if (found && selectedGrandLodge?.id !== found.id) {
        // Found a match in the list, and it's different from current local state
        console.log(`[MasonForm Effect GL Sync] Found GL '${found.name}' from context value '${contextGrandLodgeName}'. Setting local state.`);
        setSelectedGrandLodge(found);       // Set the selected object
        setGrandLodgeInputValue(found.name); // Set the input display value
      } else if (!found) {
        // Context has a value, but it's not in the current GL list (maybe list not fully loaded?)
        // For now, just set the input value to what's in context
        console.log(`[MasonForm Effect GL Sync] Could not find GL object for '${contextGrandLodgeName}'. Setting input value.`);
        setGrandLodgeInputValue(contextGrandLodgeName);
        setSelectedGrandLodge(null); // Ensure no object is selected if not found
      }
      // If found and matches selectedGrandLodge.id, do nothing - state is in sync
      
    } else if (!contextGrandLodgeName && selectedGrandLodge) {
        // Context has no value, but we have a local selection - clear local state
        console.log(`[MasonForm Effect GL Sync] Context GL is empty. Clearing local state.`);
        setSelectedGrandLodge(null);
        setGrandLodgeInputValue('');
    }
    // Add mason.grandLodge and grandLodges as dependencies
  }, [mason.grandLodge, grandLodges, selectedGrandLodge]); // Removed isLoadingGrandLodges
