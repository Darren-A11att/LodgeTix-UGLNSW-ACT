...
const MasonForm: React.FC<MasonFormProps> = ({
  ...
  // State for the 'Use same lodge' checkbox (only for additional masons)
  ...
  // Effect to initialize the 'useSameLodge' checkbox state for additional masons
  useEffect(() => {
    if (!isPrimary && mason && primaryMasonData) {
      const matchesPrimary =
        mason.grandLodgeId === primaryMasonData.grandLodgeId &&
        mason.lodgeId === primaryMasonData.lodgeId;
      // Only set initial state if lodge details actually match
      if (matchesPrimary) {
         setUseSameLodge(true);
      } else {
         // Ensure it's false if they don't match (e.g., user previously unchecked it)
         setUseSameLodge(false);
      }
    } else if (isPrimary) {
       // Ensure it's false for the primary mason
       setUseSameLodge(false);
    }
    // Depend on IDs which should be stable unless the mason record itself changes
  }, [isPrimary, mason?.grandLodgeId, mason?.lodgeId, primaryMasonData?.grandLodgeId, primaryMasonData?.lodgeId]);
  ...
  // --- Handler for 'Use Same Lodge' Checkbox ---
  const handleUseSameLodgeChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (checked && primaryMasonData) {
          // If checked, immediately update the store with primary's details
          updateAttendee(attendeeId, {
              grandLodgeId: primaryMasonData.grandLodgeId,
              lodgeId: primaryMasonData.lodgeId,
              // Use the combined display name if available
              lodgeNameNumber: primaryMasonData.lodgeNameNumber 
          });
          
          // Also clear local selections/inputs to avoid confusion
          const primaryGL = grandLodges.find(gl => gl.id === primaryMasonData.grandLodgeId);
          setSelectedGrandLodge(primaryGL || null);
          setGrandLodgeInputValue(primaryGL?.name || '');
          
          // We might not have the primary's lodge loaded, so just clear
          setSelectedLodge(null);
          setLodgeInputValue(primaryMasonData.lodgeNameNumber || ''); 
          
          // Reset lodge creation UI if it was open
          resetLodgeCreationUI();
          
      } else if (!checked) {
          // If unchecked, clear the attendee's lodge info in the store 
          // so they are forced to select manually.
          updateAttendee(attendeeId, {
              grandLodgeId: null,
              lodgeId: null,
              lodgeNameNumber: null
  ...
  }, [attendeeId, primaryMasonData, updateAttendee, grandLodges, resetLodgeCreationUI]);
