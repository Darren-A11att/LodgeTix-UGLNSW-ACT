...
const GuestPartnerForm: React.FC<GuestPartnerFormProps> = ({
  ...
  const getConfirmationMessage = () => {
    // Use primaryMasonData if available for PrimaryAttendee message
    const primaryName = primaryMasonData ? `${primaryMasonData.firstName || ''} ${primaryMasonData.lastName || ''}`.trim() : 'Primary Attendee';

    if (contactPreference === "PrimaryAttendee") {
      // Message now handled in JSX
      return ""; 
    }
    ...
    <div className="border-t border-slate-200 pt-6 mt-6 relative">
      ...
      <div className="mb-3">
        <div className="grid grid-cols-12 gap-4">
          <div className="col-span-4">
            <label className="block text-sm font-medium text-slate-700 mb-1" htmlFor={`partnerContactPreference-${id}`}>
              Contact *
          ...
          {showConfirmation && (
            <div className="col-span-8 flex items-center">
              <div className="flex items-center">
                ...
                />
                <label htmlFor={`partnerContactConfirmed-${id}`} className="ml-3 text-sm text-slate-700">
                  {contactPreference === 'PrimaryAttendee' && primaryMasonData ? (
                    <>
                      I confirm that <strong>{`${primaryMasonData.firstName || ''} ${primaryMasonData.lastName || ''}`.trim()}</strong> will be responsible for all communication with this attendee *
                    </>
                  ) : (
                    <>
